...
#get core info about the graph 
print(nx.info(G))


# In[3]:

#checking the node valye and structure 
G.nodes(data = True)[:12]
...
len(G.nodes())
#total number of nodes with Management Salary = nan
j =0
for i in range (1005):
    if(pd.isnull(G.nodes(data = True)[i][1]['ManagementSalary'])):
        j=j+1
print(j)
...
x= G.nodes(data = True)[0][1]['ManagementSalary'] == 0
y = pd.isnull(G.nodes(data = True)[1][1]['ManagementSalary'])
z = G.nodes(data = True)[3][1]['ManagementSalary']==1
print(x)
print(y)
print(z)
...
#Remember 6 features of a graph 
...
# to make a function , to check if ManagementSalary is null 
...
#preprocessing data on standard scale
from sklearn.preprocessing import MinMaxScaler
features = ['clustering', 'degree']
...
df_cn_soundarajan_hopcroft.head()    
...
from sklearn.neural_network import MLPClassifier
from sklearn.preprocessing import MinMaxScaler
def new_connections_predictionss():
